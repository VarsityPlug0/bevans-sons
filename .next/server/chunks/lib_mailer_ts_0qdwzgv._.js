...
<body style="margin:0;padding:0;background:${d};font-family:'Helvetica Neue',Helvetica,Arial,sans-serif">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:${d};padding:28px 16px">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;border-radius:16px;overflow:hidden;border:1px solid ${c}">
        ...
        <tr><td style="background:linear-gradient(90deg,${d},${s},${l},${s},${d});height:3px;font-size:0;line-height:0">&nbsp;</td></tr>
          ...
          <td style="background:${d};padding:24px 36px">
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                ...
                <td align="right">
                  <a href="${m}" style="color:${f};font-size:12px;text-decoration:none">daisygadgetsco.co.za</a>
                </td>
              </tr>
            </table>
          </td>
        ...
        <!-- Footer -->
        <tr>
          <td style="background:${d};padding:24px 36px;border-top:1px solid ${c}">
            <p style="margin:0 0 8px;color:${f};font-size:12px;text-align:center">
              Questions? &nbsp;
              <a href="https://wa.me/${x}" style="color:${s};text-decoration:none;font-weight:600">WhatsApp +27 84 896 1782</a>
              &nbsp;\xb7&nbsp;
        ...
        <!-- Bottom gold bar -->
        <tr><td style="background:linear-gradient(90deg,${d},${s},${l},${s},${d});height:2px;font-size:0;line-height:0">&nbsp;</td></tr>

      </table>
    </td></tr>
  </table>
</body>
</html>`}function $(e){return`<p style="margin:0 0 3px;color:${f};font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.12em">${e}</p>`}function w(){return`<div style="height:1px;background:${c};margin:24px 0"></div>`}function v(e,t,o=s,i=d){return`<a href="${t}" style="display:inline-block;background:${o};color:${i};font-weight:800;text-decoration:none;padding:13px 26px;border-radius:10px;font-size:14px;letter-spacing:0.02em">${e}</a>`}function z(e,t){return`<tr>
    <td style="padding:8px 0;color:${f};font-size:13px;width:150px;vertical-align:top;border-bottom:1px solid ${c}">${e}</td>
    <td style="padding:8px 0;color:#e5e7eb;font-size:13px;font-weight:600;vertical-align:top;border-bottom:1px solid ${c}">${t}</td>
  </tr>`}function k(e,t){return`<span style="display:inline-block;background:${t}22;color:${t};border:1px solid ${t}55;padding:4px 14px;border-radius:20px;font-size:12px;font-weight:700;letter-spacing:0.05em">${e}</span>`}async function C(e){let{attachments:t,cidMap:o}=await u(e.items),i=e.items.map(e=>{let t=(parseFloat(String(e.price).replace(/[^0-9.]/g,""))*e.qty).toLocaleString("en-ZA"),i=e.imageUrl?o.get(e.imageUrl)??(e.imageUrl.startsWith("http")?e.imageUrl:m+e.imageUrl):null,r=i?`<img src="${i}" alt="${e.name}" width="64" height="64" style="width:64px;height:64px;object-fit:cover;border-radius:10px;display:block;border:1px solid ${c}" />`:`<div style="width:64px;height:64px;background:${g};border:1px solid ${c};border-radius:10px"></div>`;return`
    <tr>
      <td style="padding:12px 0;border-bottom:1px solid ${c};width:76px;vertical-align:middle">${r}</td>
    ...
    <!-- Ref pill -->
    <div style="background:${g};border:1px solid ${s}44;border-radius:10px;padding:14px 20px;margin-bottom:28px;display:flex;align-items:center">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin:12px 0 28px">
      ...
      <tr>
        ...
        <td style="padding:10px 0 0;text-align:right;color:${s};font-size:20px;font-weight:900;border-top:1px solid ${c}">R ${e.total.toLocaleString("en-ZA")}</td>
      </tr>
    </table>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px">
      <tr>
        <td width="50%" style="padding:0 8px 0 0;vertical-align:top">
          <div style="background:${g};border:1px solid ${c};border-radius:10px;padding:16px 18px;margin-bottom:12px">
          ...
          <div style="background:${g};border:1px solid ${c};border-radius:10px;padding:16px 18px">
    ...
    <div style="text-align:center">
      ${v("💬 Chat on WhatsApp",`https://wa.me/${x}?text=Hi%2C%20checking%20on%20order%20${e.ref}`,"#25D366","#fff")}
    </div>
  `);await h({to:e.email,subject:`Order Confirmed ✨ — ${e.ref} | Daisy Gadgets Co.`,html:r,attachments:t})}async function A(e){let t=b(`
    ...
    <div style="background:${g};border:1px solid #22c55e44;border-left:3px solid #22c55e;border-radius:0 12px 12px 0;padding:18px 20px;margin-bottom:28px">
    ...
    ${v("Chat on WhatsApp",`https://wa.me/${x}?text=Hi%2C%20checking%20on%20order%20${e.ref}`,"#25D366","#fff")}
  `);await h({to:e.email,subject:`Payment Proof Received — ${e.ref} | Daisy Gadgets Co.`,html:t})}async function D(e){let t=e.bank??a("fnb"),{attachments:o,cidMap:i}=await u(e.items),r=e.items.map(e=>{let t=(parseFloat(String(e.price).replace(/[^0-9.]/g,""))*e.qty).toLocaleString("en-ZA"),o=parseFloat(String(e.price).replace(/[^0-9.]/g,"")).toLocaleString("en-ZA"),r=e.imageUrl?i.get(e.imageUrl)??(e.imageUrl.startsWith("http")?e.imageUrl:m+e.imageUrl):null,a=r?`<img src="${r}" alt="${e.name}" width="64" height="64" style="width:64px;height:64px;object-fit:cover;border-radius:10px;display:block;border:1px solid ${c}" />`:`<div style="width:64px;height:64px;background:${g};border:1px solid ${c};border-radius:10px"></div>`;return`
    <tr>
      <td style="padding:12px 0;border-bottom:1px solid ${c};width:76px;vertical-align:middle">${a}</td>
      <td style="padding:12px 10px;border-bottom:1px solid ${c};vertical-align:middle">
    ...
    </tr>`}).join(""),n=e.reason?`<div style="background:${g};border-left:3px solid #ef4444;border-radius:0 10px 10px 0;padding:16px 20px;margin-bottom:24px">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin:10px 0 28px">
      ...
      <tr>
        <td style="padding:10px 0 0;color:#e5e7eb;font-size:16px;font-weight:800;border-top:1px solid ${c}">Total Due</td>
        <td style="padding:10px 0 0;text-align:right;color:${s};font-size:20px;font-weight:900;border-top:1px solid ${c}">R ${e.total.toLocaleString("en-ZA")}</td>
      </tr>
    </table>
    ...
    <div style="background:${g};border:1px solid ${c};border-radius:12px;padding:4px 20px;margin-bottom:28px">
      <table width="100%" cellpadding="0" cellspacing="0">
        ${z("Bank",t.bank)}
    ...
    <div>
      ${v("📤 Upload New Proof",`${m}/checkout`,s,d)}
      &nbsp;&nbsp;
      ${v("💬 Send via WhatsApp",`https://wa.me/${x}?text=Hi%2C%20re-sending%20proof%20for%20order%20${e.ref}`,"#25D366","#fff")}
    </div>
  `);await h({to:e.email,subject:`⚠️ Action Required — ${e.ref} | Daisy Gadgets Co.`,html:p,attachments:o})}let S={approved:{pill:["Payment Approved","#22c55e"],icon:"🎊",title:"Your payment is confirmed!",body:"Great news — your payment has been verified and your order is now being packed and prepared for dispatch. We will notify you as soon as it ships.",cta:["💬 Chat on WhatsApp",`https://wa.me/${x}`]},shipped:{pill:["Shipped","#3b82f6"],icon:"📦",title:"Your order is on its way!",body:"Your order has been handed over to the courier and is heading your way. Delivery typically takes 2–5 business days within South Africa.",cta:["💬 Track via WhatsApp",`https://wa.me/${x}`]},delivered:{pill:["Delivered",s],icon:"🎁",title:"Your order has been delivered!",body:"We hope you love your new purchase! If you have any issues at all, please reach out immediately and we will make it right.",cta:["⭐ Leave a Review",`${m}/reviews`]}};async function T(e){let t=S[e.status];if(!t)return;let o=e.notes?`<div style="background:${g};border-left:3px solid ${s};border-radius:0 10px 10px 0;padding:14px 18px;margin:20px 0">
    ...
    <div style="background:${g};border:1px solid #3b82f644;border-left:3px solid #3b82f6;border-radius:0 12px 12px 0;padding:16px 20px;margin:16px 0 20px">
      <p style="margin:0 0 4px;color:${f};font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em">Tracking Number</p>
    ...
    ${o}
    ${t.cta?`<div style="margin-top:24px">${v(t.cta[0],t.cta[1])}&nbsp;&nbsp;${v("WhatsApp Us",`https://wa.me/${x}?text=Hi%2C%20re%20order%20${e.ref}`,"#25D366","#fff")}</div>`:""}
  `);await h({to:e.email,subject:i[e.status]??`Order Update — ${e.ref}`,html:r})}let R={processing:{icon:"⚙️",pillText:"Processing",pillColor:"#8b5cf6",title:"Your order is being processed",defaultMessage:"We're preparing your order for packaging. This usually takes 1–2 business days.",stage:2},packed:{icon:"📦",pillText:"Packed & Ready",pillColor:"#3b82f6",title:"Your order is packed and ready!",defaultMessage:"Your order has been carefully packed and is ready to be handed to our courier.",stage:3},dispatched:{icon:"🚚",pillText:"Dispatched",pillColor:"#3b82f6",title:"Your order has been dispatched!",defaultMessage:"Great news! Your order has been dispatched and is heading your way. Delivery typically takes 2–5 business days.",stage:4},out_for_delivery:{icon:"🏠",pillText:"Out for Delivery",pillColor:"#10b981",title:"Your order is out for delivery today!",defaultMessage:"Your order is out for delivery and should arrive today. Please ensure someone is available to receive it.",stage:5},delayed:{icon:"⏳",pillText:"Slight Delay",pillColor:"#f59e0b",title:"A small update on your order",defaultMessage:"We're experiencing a slight delay but your order is on its way. We apologise for any inconvenience and will keep you updated.",stage:-1},custom:{icon:"📬",pillText:"Update",pillColor:s,title:"An update on your order",defaultMessage:"",stage:-1}};async function P(e){var t;let o,i,r,a=R[e.templateId]??R.custom,n=e.message?`<div style="background:${g};border-left:3px solid ${a.pillColor};border-radius:0 12px 12px 0;padding:16px 20px;margin:20px 0">
        <p style="margin:0 0 4px;color:${f};font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em">Message from our team</p>
        <p style="margin:0;color:#d1d5db;font-size:14px;line-height:1.7">${e.message.replace(/\n/g,"<br>")}</p>
       </div>`:"",p=e.tracking_number?`<div style="background:${g};border:1px solid #3b82f644;border-left:3px solid #3b82f6;border-radius:0 12px 12px 0;padding:16px 20px;margin:16px 0">
        <p style="margin:0 0 4px;color:${f};font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em">Tracking Number</p>
        <p style="margin:0;color:#93c5fd;font-size:18px;font-weight:700;font-family:monospace;letter-spacing:0.06em">${e.tracking_number}</p>
       </div>`:"",l=b(`
    ...
    <p style="margin:0 0 4px;color:${f};font-size:13px">Order: <strong style="color:${s}">${e.ref}</strong></p>
    ${w()}
    <p style="margin:0 0 4px;color:#9ca3af;font-size:15px">Hi ${e.name.split(" ")[0]},</p>
    ${a.stage>0?(t=a.stage,i=[],r=[],(o=["Order Placed","Processing","Packed","Dispatched","Delivered"]).forEach((e,a)=>{let n=a+1,p=n<t,l=n===t,g=p?"#10b981":l?s:"#1a1a1a",c=p?"#fff":l?d:"#555";i.push(`<td align="center"><table cellpadding="0" cellspacing="0" style="margin:0 auto"><tr><td align="center" width="28" height="28" style="width:28px;height:28px;border-radius:14px;background:${g};border:2px solid ${p?"#10b981":l?s:"#2a2a2a"};text-align:center;vertical-align:middle;font-size:11px;font-weight:800;color:${c};line-height:24px">${p?"&#10003;":n}</td></tr></table></td>`),r.push(`<td align="center" style="padding:6px 2px 0;vertical-align:top"><p style="margin:0;font-size:10px;color:${l?"#e5e7eb":p?"#9ca3af":"#4b5563"};font-weight:${l?700:400};line-height:1.4">${e}</p></td>`),a<o.length-1&&(i.push(`<td style="vertical-align:middle;padding-bottom:4px"><div style="height:2px;background:${a+1<t?"#10b981":"#2a2a2a"}"></div></td>`),r.push("<td></td>"))}),`<table width="100%" cellpadding="0" cellspacing="0" style="margin:20px 0 8px"><tr>${i.join("")}</tr><tr>${r.join("")}</tr></table>`):""}
    ${n}
    ${p}
    <div style="margin-top:24px">
      ${v("💬 Chat on WhatsApp",`https://wa.me/${x}?text=Hi%2C%20checking%20on%20order%20${e.ref}`,"#25D366","#fff")}
    </div>
  `);await h({to:e.email,subject:`Order Update — ${e.ref} | Daisy Gadgets Co.`,html:l})}async function Y(e){let t=b(`
    ${$("Your Quote is Ready")}
    ...
    <!-- Package card -->
    <div style="background:${g};border:1px solid ${s}44;border-radius:14px;padding:28px;margin-bottom:24px;text-align:center">
      ${$("Recommended Package")}
    ...
    <div style="background:${g};border:1px solid ${c};border-left:3px solid ${s};border-radius:0 12px 12px 0;padding:16px 20px;margin-bottom:28px">
    ...
    ${v("Accept Quote",`https://wa.me/${x}?text=Hi%2C%20I%20accept%20quote%20${e.ref}`,s,d)}
    &nbsp;&nbsp;
    ${v("Ask a Question",`https://wa.me/${x}?text=Hi%2C%20question%20about%20quote%20${e.ref}`,"#25D366","#fff")}
  `);await h({to:e.email,subject:`Your Quote — ${e.ref} | Daisy Gadgets Co.`,html:t})}async function H(e){let t=b(`
    ...
    <!-- Code card -->
    <div style="background:${d};border:1px solid ${s}55;border-radius:14px;padding:32px;text-align:center;margin-bottom:28px">
    ...
    ${v("🛍️ Shop Now",`${m}/shop`)}
    &nbsp;&nbsp;
    ${v("💬 Claim via WhatsApp",`https://wa.me/${x}?text=Hi%2C%20I%20have%20the%20discount%20code%20DAISY25`,"#25D366","#fff")}
  `);await h({to:e.email,subject:"✨ Your 25% Discount Code — Daisy Gadgets Co.",html:t})}e.s(["TRACKING_TEMPLATES",0,R,"sendMail",0,h,"sendOrderConfirmation",0,C,"sendProofAcknowledgement",0,A,"sendQuoteReply",0,Y,"sendRejectionEmail",0,D,"sendStatusUpdate",0,T,"sendTrackingUpdate",0,P,"sendWelcomeEmail",0,H],67010)}];
