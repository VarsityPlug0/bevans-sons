...
<body style="margin:0;padding:0;background:${d};font-family:'Helvetica Neue',Helvetica,Arial,sans-serif">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:${d};padding:28px 16px">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;border-radius:16px;overflow:hidden;border:1px solid ${g}">
        ...
        <tr><td style="background:linear-gradient(90deg,${d},${l},${s},${l},${d});height:3px;font-size:0;line-height:0">&nbsp;</td></tr>
        ...
        <tr>
          <td style="background:${d};padding:24px 36px;border-top:1px solid ${g}">
            <p style="margin:0 0 8px;color:${m};font-size:12px;text-align:center">
              Questions? &nbsp;
              <a href="https://wa.me/${f}" style="color:${l};text-decoration:none;font-weight:600">WhatsApp +27 82 587 6811</a>
              &nbsp;\xb7&nbsp;
              <a href="${x}" style="color:${l};text-decoration:none;font-weight:600">daisygadgetsco.com</a>
            </p>
    ...
    <tr>
      ...
      <td style="padding:12px 0;border-bottom:1px solid ${g};text-align:right;vertical-align:middle">
        <span style="color:${l};font-size:14px;font-weight:700">R ${t}</span>
      </td>
    </tr>`}).join(""),r=b(`
    ...
    <!-- Ref pill -->
    <div style="background:${c};border:1px solid ${l}44;border-radius:10px;padding:14px 20px;margin-bottom:28px;display:flex;align-items:center">
      <table width="100%" cellpadding="0" cellspacing="0"><tr>
        ...
        <td style="text-align:right;color:${l};font-size:18px;font-weight:900;letter-spacing:0.08em;font-family:monospace">${e.ref}</td>
      </tr></table>
    </div>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin:12px 0 28px">
      ...
      <tr>
        ...
        <td style="padding:10px 0 0;text-align:right;color:${l};font-size:20px;font-weight:900;border-top:1px solid ${g}">R ${e.total.toLocaleString("en-ZA")}</td>
      </tr>
    </table>
  ...
  `);await y({to:e.email,subject:`Order Confirmed ✨ — ${e.ref} | Daisy Gadgets Co.`,html:r,attachments:t})}async function D(e){let t=b(`
    ...
    <p style="margin:0 0 24px;color:#9ca3af;font-size:15px;line-height:1.6">
      Hi ${e.name.split(" ")[0]}, we received your proof of payment for order <strong style="color:${l}">${e.ref}</strong>.
    </p>
  ...
  `);await y({to:e.email,subject:`Payment Proof Received — ${e.ref} | Daisy Gadgets Co.`,html:t})}async function C(e){let t=e.bank??n("tymebank"),{attachments:o,cidMap:i}=await u(e.items),r=e.items.map(e=>{let t=(parseFloat(String(e.price).replace(/[^0-9.]/g,""))*e.qty).toLocaleString("en-ZA"),o=parseFloat(String(e.price).replace(/[^0-9.]/g,"")).toLocaleString("en-ZA"),r=e.imageUrl?i.get(e.imageUrl)??(e.imageUrl.startsWith("http")?e.imageUrl:x+e.imageUrl):null,n=r?`<img src="${r}" alt="${e.name}" width="64" height="64" style="width:64px;height:64px;object-fit:cover;border-radius:10px;display:block;border:1px solid ${g}" />`:`<div style="width:64px;height:64px;background:${c};border:1px solid ${g};border-radius:10px"></div>`;return`
    ...
    <div style="text-align:center;margin-bottom:28px">
      ...
      <p style="margin:0;color:#9ca3af;font-size:14px;line-height:1.6">Hi <strong style="color:#e5e7eb">${e.name.split(" ")[0]}</strong>, we were unable to verify your proof of payment for order <strong style="color:${l};font-family:monospace">${e.ref}</strong>.</p>
    </div>
    ...
    <div style="background:${c};border:1px solid ${g};border-radius:12px;padding:4px 20px;margin-bottom:28px">
      <table width="100%" cellpadding="0" cellspacing="0">
        ...
        ${z("Account Number",`<span style="font-family:monospace;font-size:15px;color:${l};letter-spacing:0.06em">${t.accountNumber}</span>`)}
        ${z("Branch Code",t.branchCode)}
        ${t.payshap?z("PayShap",t.payshap):""}
        ${z("Reference",`<strong style="color:${l};font-size:15px;font-family:monospace">${e.ref}</strong>`)}
        ${z("Amount",`<strong style="color:${l};font-size:15px">R ${e.total.toLocaleString("en-ZA")}</strong>`)}
      </table>
    </div>
  ...
  `);await y({to:e.email,subject:`⚠️ Action Required — ${e.ref} | Daisy Gadgets Co.`,html:p,attachments:o})}let U={approved:{pill:["Payment Approved","#22c55e"],icon:"🎊",title:"Your payment is confirmed!",body:"Great news — your payment has been verified and your order is now being packed and prepared for dispatch. We will notify you as soon as it ships.",cta:["💬 Chat on WhatsApp",`https://wa.me/${f}`]},shipped:{pill:["Shipped","#3b82f6"],icon:"📦",title:"Your order has been shipped!",body:"We are pleased to inform you that your order has been successfully packed, processed and shipped.\n\nYour parcel is now in transit to the selected delivery destination. Please keep your contact number available in case our delivery team needs to contact you regarding your order.\n\nWe will notify you again when your order moves to Out for Delivery.",cta:["💬 Track via WhatsApp",`https://wa.me/${f}`]},delivered:{pill:["Delivered",l],icon:"🎁",title:"Your order has been delivered!",body:"We are delighted to confirm that your Daisy Gadgets Co. order has been successfully delivered.\n\nThank you for trusting Daisy Gadgets Co. with your purchase. We hope you are completely satisfied with your order. If you experience any issue with the product or require assistance after delivery, please contact our customer support team and we will be happy to assist.\n\nWe would also appreciate your feedback about your shopping experience with us.\n\nThank you for choosing Daisy Gadgets Co. — Smart Tech. Better Living.",cta:["⭐ Leave a Review",`${x}/reviews`]}};async function P(e){let t=U[e.status];if(!t)return;let o=e.notes?`<div style="background:${c};border-left:3px solid ${l};border-radius:0 10px 10px 0;padding:14px 18px;margin:20px 0">
       ...
       </div>`:"",r="delivered"===e.status?`<div style="background:${c};border:1px solid ${l}33;border-radius:12px;padding:16px 20px;margin:4px 0 20px">
        <table width="100%" cellpadding="0" cellspacing="0">
          <tr>
    ...
    <p style="margin:0 0 4px;color:${m};font-size:13px">Order: <strong style="color:${l}">${e.ref}</strong></p>
    ${v()}
  ...
  `);await y({to:e.email,subject:n.subject,html:s})}async function I(e){let t=b(`
    ...
    <!-- Package card -->
    <div style="background:${c};border:1px solid ${l}44;border-radius:14px;padding:28px;margin-bottom:24px;text-align:center">
      ${$("Recommended Package")}
      ...
      ${$("Estimated Price")}
      <p style="margin:8px 0 0;color:${l};font-size:34px;font-weight:900;letter-spacing:0.02em">${e.price}</p>
    </div>

    ${e.message?`
    <p style="margin:0 0 10px;color:#e5e7eb;font-size:15px;font-weight:700">Message from our team</p>
    <div style="background:${c};border:1px solid ${g};border-left:3px solid ${l};border-radius:0 12px 12px 0;padding:16px 20px;margin-bottom:28px">
    ...
    ${w("Accept Quote",`https://wa.me/${f}?text=Hi%2C%20I%20accept%20quote%20${e.ref}`,l,d)}
    &nbsp;&nbsp;
    ${w("Ask a Question",`https://wa.me/${f}?text=Hi%2C%20question%20about%20quote%20${e.ref}`,"#25D366","#fff")}
  `);await y({to:e.email,subject:`Your Quote — ${e.ref} | Daisy Gadgets Co.`,html:t})}async function T(e){let t=b(`
    ...
    <!-- Code card -->
    <div style="background:${d};border:1px solid ${l}55;border-radius:14px;padding:32px;text-align:center;margin-bottom:28px">
      ${$("Your Exclusive Discount Code")}
      <p style="margin:12px 0;color:${l};font-size:40px;font-weight:900;letter-spacing:0.15em;font-family:monospace">DAISY25</p>
  ...
  `,r=b(`
    ...
    <!-- Ref + product -->
    <div style="background:${d};border:1px solid ${l}44;border-radius:12px;padding:16px 20px;margin-bottom:24px">
      ${$("Application Reference")}
      <p style="margin:4px 0 12px;color:${l};font-size:24px;font-weight:900;font-family:monospace;letter-spacing:0.1em">${e.ref}</p>
      ${$("Product")}
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px">
      ...
      <tr>
        ...
        <td style="padding:10px 0;border-bottom:1px solid ${g};color:${l};font-size:18px;font-weight:900;text-align:right">${t(e.monthly_payment)}/mo</td>
      </tr>
      <tr>
    ...
    <div style="background:#f59e0b11;border:1px solid #f59e0b44;border-radius:12px;padding:16px 20px;margin-bottom:24px">
      ...
      <p style="margin:0;color:#d1d5db;font-size:13px;line-height:1.6">Transfer <strong style="color:#f59e0b">${t(e.deposit)}</strong> to one of our accounts below using <strong style="color:#fff">${e.ref}</strong> as your payment reference, then send proof of payment on WhatsApp to activate your plan.</p>
    </div>
  ...
  `);await y({to:e.email,subject:`✅ Installment Approved — ${e.ref} | Daisy Gadgets Co.`,html:r})}function _(e,t,o){return`
    ...
    <div style="background:${d};border:1px solid ${l}44;border-radius:12px;padding:16px 20px;margin-bottom:24px">
      ${$("Application Reference")}
      <p style="margin:4px 0 12px;color:${l};font-size:24px;font-weight:900;font-family:monospace;letter-spacing:0.1em">${e}</p>
      ${$("Product")}
  ...
  `);await y({to:e.email,subject:`Application Under Review — ${e.ref} | Daisy Gadgets Co.`,html:o})}async function G(e){var t,o,i,r,n;let a,p,s=encodeURIComponent(`Hi, I am sending proof of payment for my installment deposit. Application: ${e.ref} — ${e.product_name}.`),c=b(`
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px">
      ...
      <tr>
        ...
        <td style="padding:10px 0;border-bottom:1px solid ${g};color:${l};font-size:16px;font-weight:900;text-align:right">${p(o)}/mo</td>
      </tr>
      <tr>
        <td style="padding:10px 0;color:${m};font-size:13px">Total Repayable</td>
    ...
    <div style="text-align:center">
      ${w("Send Proof of Payment",`https://wa.me/${f}?text=${s}`,"#25D366","#fff")}
    </div>
  ...
  `);await y({to:e.email,subject:`Deposit Required — ${e.ref} | Daisy Gadgets Co.`,html:c})}async function H(e){let t=e=>`R ${e.toLocaleString("en-ZA",{minimumFractionDigits:2})}`,o=encodeURIComponent(`Hi, I would like to check on my active installment plan ${e.ref} for the ${e.product_name}.`),i=b(`
    ...
    <div style="background:${d};border:1px solid ${g};border-radius:12px;padding:16px 20px;margin-bottom:24px">
      ...
      <p style="margin:0 0 8px;color:#d1d5db;font-size:13px;line-height:1.6">Make your monthly payment to the same bank account using <strong style="color:${l}">${e.ref}</strong> as your reference.</p>
  ...
  `);await y({to:e.email,subject:`Plan Activated — ${e.ref} | Daisy Gadgets Co.`,html:i})}async function O(e){let t,o=encodeURIComponent(`Hi, I would like to enquire about another product on installments. My previous plan was ${e.ref}.`),i=b(`
    ...
    <div style="background:#D4AF3711;border:1px solid #D4AF3744;border-radius:12px;padding:20px;margin-bottom:24px;text-align:center">
      ${$("Total Paid")}
      <p style="margin:8px 0 4px;color:${l};font-size:36px;font-weight:900">${(t=e.total_repayable,`R ${t.toLocaleString("en-ZA",{minimumFractionDigits:2})}`)}</p>
  ...
  `);await y({to:e.email,subject:`Application Update — ${e.ref} | Daisy Gadgets Co.`,html:o})}async function E(e){let{attachments:t,cidMap:o}=await u(e.items),i=e.items.map(e=>{let t=e.imageUrl?o.get(e.imageUrl)??(e.imageUrl.startsWith("http")?e.imageUrl:x+e.imageUrl):null,i=t?`<img src="${t}" alt="${e.name}" width="64" height="64" style="width:64px;height:64px;object-fit:cover;border-radius:10px;display:block;border:1px solid ${g}" />`:`<div style="width:64px;height:64px;background:${c};border:1px solid ${g};border-radius:10px"></div>`;return`
    <tr>
      ...
      <td style="padding:10px 0;border-bottom:1px solid ${g};text-align:right;vertical-align:middle">
        <span style="color:${l};font-size:13px;font-weight:700">${e.price}</span>
      </td>
    </tr>`}).join(""),r=`
    <h1 style="margin:0 0 6px;color:#fff;font-size:24px;font-weight:800;letter-spacing:-0.02em">Order placed!</h1>
    <p style="margin:0 0 24px;color:${m};font-size:15px">Hi ${e.name.split(" ")[0]}, your order <strong style="color:${l}">${e.ref}</strong> is in — we're waiting for your proof of payment.</p>
    ...
    <p style="margin:0;color:${m};font-size:13px;text-align:center">
      Questions? ${w("WhatsApp Us",`https://wa.me/${f}?text=Hi%2C%20my%20order%20ref%20is%20${encodeURIComponent(e.ref)}`,c,l)}
    </p>
  `;await y({to:e.email,subject:`Order ${e.ref} received — clear your cart | Daisy Gadgets Co.`,html:b(r),attachments:t})}async function M(e){let t=e.ctaUrl&&e.trackingId?`${x}/api/track/email?id=${e.trackingId}&e=click&url=${encodeURIComponent(e.ctaUrl)}`:e.ctaUrl,o=e.ctaText&&t?`<div style="text-align:center;margin:28px 0">${w(e.ctaText,t)}</div>`:"",i=e.trackingId?`<img src="${x}/api/track/email?id=${e.trackingId}&e=open" width="1" height="1" style="display:none;width:1px;height:1px;border:0" alt="" />`:"",r="",n=[];if(e.orderItems?.length){let{attachments:t,cidMap:o}=await u(e.orderItems.map(e=>({name:e.name,imageUrl:e.imageUrl})));n=t;let i=e.orderItems.map(e=>{let t=e.imageUrl?o.get(e.imageUrl)??(e.imageUrl.startsWith("http")?e.imageUrl:x+e.imageUrl):null,i=t?`<img src="${t}" alt="${e.name}" width="64" height="64" style="width:64px;height:64px;object-fit:cover;border-radius:10px;display:block;border:1px solid ${g}" />`:`<div style="width:64px;height:64px;background:${c};border:1px solid ${g};border-radius:10px"></div>`,r=e.id?`${x}/shop/${e.id}`:`${x}/shop`;return`
      <tr>
      ...
      </tr>`}).join(""),a=e.orderRef?`<p style="margin:0 0 14px;color:${m};font-size:12px">Order ref: <span style="color:${l};font-weight:700;font-family:monospace">${e.orderRef}</span></p>`:"",p=e.restoreCartUrl?`<div style="text-align:center;margin-top:20px">${w("Complete Your Order →",e.restoreCartUrl)}</div>`:"";r=`
      ${v()}
      ...
      ${p}`}else if(e.featuredProducts?.length){let{attachments:t,cidMap:o}=await u(e.featuredProducts.map(e=>({name:e.name,imageUrl:e.imageUrl})));n=t;let i=e.featuredProducts.map(e=>{let t=e.imageUrl?o.get(e.imageUrl)??(e.imageUrl.startsWith("http")?e.imageUrl:x+e.imageUrl):null,i=t?`<img src="${t}" alt="${e.name}" width="200" style="width:100%;max-width:200px;height:140px;object-fit:cover;border-radius:10px;display:block;border:1px solid ${g}" />`:`<div style="width:100%;height:140px;background:${c};border:1px solid ${g};border-radius:10px"></div>`;return`
        <td style="width:48%;vertical-align:top;padding:6px">
          <a href="${x}/shop/${e.id}" style="text-decoration:none;display:block">
            ...
            <p style="margin:0;color:${l};font-size:14px;font-weight:800">${e.price}</p>
          </a>
    ...
    ${i}
  `;await y({to:e.to,subject:e.subject,html:b(a),attachments:n.length?n:void 0})}e.s(["TRACKING_TEMPLATES",0,R,"sendCampaignEmail",0,M,"sendClearCartReminder",0,E,"sendInstallmentActive",0,H,"sendInstallmentApproval",0,W,"sendInstallmentAwaitingPayment",0,G,"sendInstallmentCompleted",0,O,"sendInstallmentDeclined",0,B,"sendInstallmentReviewing",0,Y,"sendMail",0,y,"sendOrderConfirmation",0,A,"sendProofAcknowledgement",0,D,"sendQuoteReply",0,I,"sendRejectionEmail",0,C,"sendStatusUpdate",0,P,"sendTrackingUpdate",0,S,"sendWelcomeEmail",0,T],67010)}];
