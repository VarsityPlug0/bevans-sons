module.exports=[67010,e=>{"use strict";var t=e.i(84423),o=e.i(14747),i=e.i(22734);let r=o.default.join(process.cwd(),"public","logo.jpg"),a="logo@daisygadgets",n="#D4AF37",p="#f5d76e",s="#0A0A0A",d="#161616",l="#1F1F1F",g="#6b7280",c="27848961782",x="https://daisygadgetsco.co.za";async function f(e){let o,n,p=(o=process.env.MAIL_USER,n=process.env.MAIL_PASS,o&&n?t.default.createTransport({service:"gmail",auth:{user:o,pass:n}}):null);if(!p)return void console.error("mailer: env vars missing");try{let t=e.attachments??[];(0,i.existsSync)(r)&&t.unshift({filename:"logo.jpg",path:r,cid:a}),await p.sendMail({from:`"Daisy Gadgets Co." <${process.env.MAIL_USER}>`,to:e.to,subject:e.subject,html:e.html,attachments:t})}catch(e){console.error("mailer send error:",e)}}async function m(e){try{let t=await fetch(e,{signal:AbortSignal.timeout(5e3)});if(!t.ok)return null;return Buffer.from(await t.arrayBuffer())}catch{return null}}async function h(e){let t=[],o=new Map;return await Promise.all(e.map(async(e,i)=>{if(!e.imageUrl)return;let r=await m(e.imageUrl.startsWith("http")?e.imageUrl:x+e.imageUrl);if(!r)return;let a=`product-${i}@daisy`,n=e.imageUrl.split(".").pop()?.split("?")[0]??"jpg";t.push({filename:`product-${i}.${n}`,content:r,cid:a}),o.set(e.imageUrl,`cid:${a}`)})),{attachments:t,cidMap:o}}function y(e,t=""){return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
