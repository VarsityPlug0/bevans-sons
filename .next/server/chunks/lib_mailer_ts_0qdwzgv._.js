...
<body style="margin:0;padding:0;background:${d};font-family:'Helvetica Neue',Helvetica,Arial,sans-serif">
  ...
  `);await y({to:e.email,subject:n.subject,html:l})}async function I(e){let t=b(`
    ${$("Your Quote is Ready")}
  ...
  `,r=b(`
    ...
    <div style="background:${d};border:1px solid ${s}44;border-radius:12px;padding:16px 20px;margin-bottom:24px">
      ...
      <p style="margin:4px 0 12px;color:${s};font-size:24px;font-weight:900;font-family:monospace;letter-spacing:0.1em">${e.ref}</p>
      ${$("Product")}
  ...
  `);await y({to:e.email,subject:`Deposit Required — ${e.ref} | Daisy Gadgets Co.`,html:c})}async function G(e){let t=e=>`R ${e.toLocaleString("en-ZA",{minimumFractionDigits:2})}`,o=encodeURIComponent(`Hi, I would like to check on my active installment plan ${e.ref} for the ${e.product_name}.`),i=b(`
    ...
    <div style="background:${d};border:1px solid ${g};border-radius:12px;padding:16px 20px;margin-bottom:24px">
      ...
      <p style="margin:0 0 8px;color:#d1d5db;font-size:13px;line-height:1.6">Make your monthly payment to the same bank account using <strong style="color:${s}">${e.ref}</strong> as your reference.</p>
  ...
  `);await y({to:e.email,subject:`Plan Activated — ${e.ref} | Daisy Gadgets Co.`,html:i})}async function H(e){let t,o=encodeURIComponent(`Hi, I would like to enquire about another product on installments. My previous plan was ${e.ref}.`),i=b(`
    ...
    <div style="background:#D4AF3711;border:1px solid #D4AF3744;border-radius:12px;padding:20px;margin-bottom:24px;text-align:center">
      ${$("Total Paid")}
      <p style="margin:8px 0 4px;color:${s};font-size:36px;font-weight:900">${(t=e.total_repayable,`R ${t.toLocaleString("en-ZA",{minimumFractionDigits:2})}`)}</p>
  ...
  `;await y({to:e.email,subject:`Order ${e.ref} received — clear your cart | Daisy Gadgets Co.`,html:b(r),attachments:t})}async function F(e){let t=e.ctaUrl&&e.trackingId?`${x}/api/track/email?id=${e.trackingId}&e=click&url=${encodeURIComponent(e.ctaUrl)}`:e.ctaUrl,o=e.ctaText&&t?`<div style="text-align:center;margin:28px 0">${w(e.ctaText,t)}</div>`:"",i=e.trackingId?`<img src="${x}/api/track/email?id=${e.trackingId}&e=open" width="1" height="1" style="display:none;width:1px;height:1px;border:0" alt="" />`:"",r="",n=[];if(e.orderItems?.length){let{attachments:t,cidMap:o}=await u(e.orderItems.map(e=>({name:e.name,imageUrl:e.imageUrl})));n=t;let i=e.orderItems.map(e=>{let t=e.imageUrl?o.get(e.imageUrl)??(e.imageUrl.startsWith("http")?e.imageUrl:x+e.imageUrl):null,i=t?`<img src="${t}" alt="${e.name}" width="64" height="64" style="width:64px;height:64px;object-fit:cover;border-radius:10px;display:block;border:1px solid ${g}" />`:`<div style="width:64px;height:64px;background:${c};border:1px solid ${g};border-radius:10px"></div>`,r=e.id?`${x}/shop/${e.id}`:`${x}/shop`;return`
      ...
      </tr>`}).join(""),a=e.orderRef?`<p style="margin:0 0 14px;color:${m};font-size:12px">Order ref: <span style="color:${s};font-weight:700;font-family:monospace">${e.orderRef}</span></p>`:"",p=e.restoreCartUrl?`<div style="text-align:center;margin-top:20px">${w("Complete Your Order →",e.restoreCartUrl)}</div>`:"";r=`
      ${v()}
    ...
    ${i}
  `;await y({to:e.to,subject:e.subject,html:b(a),attachments:n.length?n:void 0})}e.s(["TRACKING_TEMPLATES",0,P,"sendCampaignEmail",0,F,"sendClearCartReminder",0,N,"sendInstallmentActive",0,G,"sendInstallmentApproval",0,W,"sendInstallmentAwaitingPayment",0,B,"sendInstallmentCompleted",0,H,"sendInstallmentDeclined",0,O,"sendInstallmentReviewing",0,Y,"sendMail",0,y,"sendOrderConfirmation",0,A,"sendProofAcknowledgement",0,D,"sendQuoteReply",0,I,"sendRejectionEmail",0,C,"sendStatusUpdate",0,R,"sendTrackingUpdate",0,S,"sendWelcomeEmail",0,T],67010)}];
