module.exports=[66879,a=>{"use strict";var b=a.i(85148),c=a.i(14747),d=a.i(22734);let e=process.env.DATA_DIR??c.default.join(process.cwd(),"data"),f=c.default.join(e,"bevans.db"),g=null;a.s(["getDb",0,function(){var a;let c;return g||((0,d.existsSync)(e)||(0,d.mkdirSync)(e,{recursive:!0}),(g=new b.default(f)).pragma("journal_mode = WAL"),g.pragma("foreign_keys = ON"),(a=g).exec(`
  ...
  `),(c=b=>{try{a.exec(b)}catch{}})("ALTER TABLE orders ADD COLUMN bank_id TEXT"),c("ALTER TABLE orders ADD COLUMN tracking_number TEXT"),c("ALTER TABLE installment_applications ADD COLUMN product_imageUrl TEXT"),function(a){var b;let c,d,e,f,g=b=>!!a.prepare("SELECT name FROM migrations WHERE name = ?").get(b),h=b=>a.prepare("INSERT OR IGNORE INTO migrations (name) VALUES (?)").run(b),i=b=>{try{a.exec(b)}catch{}};if(g("add_bevans_product_columns_v1")||(i("ALTER TABLE products ADD COLUMN slug TEXT NOT NULL DEFAULT ''"),i("ALTER TABLE products ADD COLUMN gender TEXT NOT NULL DEFAULT 'Unisex'"),i("ALTER TABLE products ADD COLUMN material TEXT NOT NULL DEFAULT ''"),i("ALTER TABLE products ADD COLUMN fit TEXT NOT NULL DEFAULT ''"),i("ALTER TABLE products ADD COLUMN newArrival INTEGER NOT NULL DEFAULT 0"),h("add_bevans_product_columns_v1")),g("create_product_variants_v1")||(a.exec(`
    ...
    `),h("create_product_variants_v1")),g("create_product_images_v1")||(a.exec(`
    ...
    `),h("create_product_images_v1")),!g("drop_solar_quotes_v1"))if(a.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='quotes'").get()){let b=a.prepare("SELECT COUNT(*) as c FROM quotes").get().c;b>0?console.warn(`[bevans] quotes table has ${b} row(s) — skipping drop. Manual review needed.`):(a.exec("DROP TABLE IF EXISTS quotes"),h("drop_solar_quotes_v1"))}else h("drop_solar_quotes_v1");g("seed_bevans_products_v1")||(0===a.prepare("SELECT COUNT(*) as c FROM products").get().c&&(b=a,c=new Date().toISOString(),d=b.prepare(`
