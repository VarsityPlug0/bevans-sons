...
import { MEN_CATEGORIES } from "@/lib/categories";
import { ArrowRight } from "lucide-react";

export const metadata: Metadata = {
  title: `Men's Clothing | ${BRAND.name}`,
  description: "Shop men's hoodies, T-shirts, shirts, jackets, pants and shorts. Premium quality, free delivery in South Africa.",
  alternates: { canonical: `${BRAND.domain}/men` },
};

const CAT_LINKS = MEN_CATEGORIES.map((c) => ({
  label: c,
  href: `/shop?cat=${encodeURIComponent(c)}`,
}));

export default function MenPage() {
  const products = getProductsByGender("Men", 24);
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative py-24 px-6 bg-[#0A0A0A] border-b border-[#1F1F1F] text-center">
        <p className="section-label mb-3">Men&apos;s Collection</p>
        <h1
        ...
        <p className="text-gray-400 max-w-md mx-auto mb-8">
          Hoodies, tees, shirts, jackets and more — designed for men who wear their confidence.
        </p>
        <Link href="/shop?gender=Men" className="btn-primary px-8 py-3.5 rounded-xl font-bold">
          Shop All Men&apos;s
        </Link>
      </section>
      ...
      <section className="px-4 sm:px-6 pb-20 max-w-6xl mx-auto">
        {products.length === 0 ? (
          <div className="py-24 text-center">
            <p className="text-gray-500 mb-6">No men&apos;s products yet — check back soon.</p>
        ...
        <div className="mt-10 text-center">
          <Link href="/shop?gender=Men" className="text-sm text-gray-400 hover:text-white flex items-center gap-1 justify-center transition-colors">
            View full men&apos;s catalogue <ArrowRight size={14} />
          </Link>
        </div>
      </section>
